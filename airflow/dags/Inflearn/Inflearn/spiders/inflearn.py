...
class InflearnSpider(scrapy.Spider):
    ...
    def parse_items(self, response):
        ...
        pattern = re.compile(r'[^a-zA-Zㄱ-ㅎㅏ-ㅣ가-힣\s.,;!?\'\"@#\$%\^&\*\(\)\[\]\{\}\-_+=]')

        # 시간 지연..
        category_name = pattern.sub('', category_name)
        sub_category_name = pattern.sub('', sub_category_name)
        title = pattern.sub('', title)
        for nol in num_of_lecture:
            nol = pattern.sub('', nol)
        for c in curri:
            c = pattern.sub('', c)
        instructor = pattern.sub('', instructor)
        for id in instructor_desc:
            id = pattern.sub('', id)
        for t in tag:
            t = pattern.sub('', t)
        for r in review:
